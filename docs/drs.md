...
| 2.3     | 16/06/27 | Davide Ottone Casagrande | refactor: separate concerns between main and config for better encapsulation                    |
...
classDiagram
  ...
  class Config {
    +str folder_path
    +str export_path
    +str elasticsearch_address
    +str index_name
    +dict mapping
    +int delta_T_for_file
    -str destinations
    +__init_(section str)
  }
  ...
  class DataStore {
    ...
    -Elasticsearch es
    -str index_name
    -Path file_system_export
    +__init__(str export_path, str elasticsearch_address, str index_name) None
    +create_mapping(dict mapping) None
    +insert(str) None
  }
...
  }
  ...
  class Logsfile {
    ...
    -int deltaT
    -int number_of_logs
    +__generate_random_timestamps(datetime startingHour) list[str]
  }
:::
